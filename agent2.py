...
class LinkEvaluator:
    ...
    async def evaluate_batch(self, topic: str, links: List[Dict]) -> List[Dict]:
            """Evaluate a batch of links at once (Massive Speedup)"""
            # 1. Format links for the prompt
            links_text = ""
            for i, link in enumerate(links):
                links_text += f"ID {i}: {link['text']} ({link['url']})\n"

            # 2. Batch Prompt
            prompt = f"""You are a research filter. I will give you a list of links.
            Identify which ones are HIGHLY RELEVANT to the topic: "{topic}"
            ...
            2. Ignore general navigation (Home, About), login pages, or unrelated news.
            3. Be strict. If unsure, exclude it.

            RESPONSE FORMAT (JSON ONLY):
            {{
                "relevant_ids": [0, 2, 5],
                "reasoning": {{
                    "0": "Directly mentions topic",
                    "2": "Title implies research paper",
                    "5": "Relevant tutorial"
                }}
            }}
            """
            ...
            # 4. Map results back to links
            results = []
            relevant_ids = response.get("relevant_ids", []) if response else []
            reasons = response.get("reasoning", {}) if response else {}

            for i, link in enumerate(links):
                is_relevant = i in relevant_ids
                results.append({
                    "should_click": is_relevant,
    ...
    def heuristic_filter(self, topic: str, link_text: str) -> float:
            """Fast keyword check before AI"""
            score = 0.0
            ...
            if topic.lower() in text_lower:
                score += 0.5
            elif any(w in text_lower for w in topic_words if len(w) > 3):
                score += 0.3
                
            # 2. Negative Keywords (Skip login, privacy policy, etc)
            negatives = ['login', 'sign up', 'forgot password', 'privacy policy', 'terms', 'cart', 'checkout']
    ...
    async def research(self, topic: str, start_url: str) -> List[ResearchResult]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Step 5: BATCHED EVALUATION (NEW)
                logger.log("thinking", f"🤔 Batch evaluating {len(links_to_evaluate)} links...")
                promising_links = []
                
                # Process in chunks of 15
                chunk_size = 15
                for i in range(0, len(links_to_evaluate), chunk_size):
                    chunk = links_to_evaluate[i:i + chunk_size]
                    
                    # 1. Filter out visited/bad links before asking AI
                    valid_chunk = []
                    for link in chunk:
                        if link['url'] in self.visited_urls: continue
                        text_lower = link['text'].lower()
                        if len(text_lower) < 10 or any(x in text_lower for x in ['login', 'sign up', 'contact', 'about']):
                            continue
                        valid_chunk.append(link)

                    if not valid_chunk: continue

                    logger.log("info", f"  ⚡ Analyzing batch {i//chunk_size + 1} ({len(valid_chunk)} links)...")

                    # 2. Call the batch evaluator
                    try:
                        batch_results = await self.evaluator.evaluate_batch(topic, valid_chunk)
                        
                        # 3. Process results
                        for link, decision in zip(valid_chunk, batch_results):
                            if decision.get('should_click', False):
                                ...
                                logger.log("decision", f"    ✓ FOUND: {link['text'][:40]}...")
                    except Exception as e:
                        logger.log("error", f"Batch error: {e}")
