        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for i in range(0, len(links_to_evaluate), chunk_size):
                    ...
                    if self.ui_callback:
                        self.ui_callback('progress', {
                            'stage': "Analyzing Batches",
                            'current': i + len(chunk),
                            'total': len(links_to_evaluate),
                            'message': f"Scanning batch {(i//chunk_size)+1}..."
                        })
                    ...
                    for link in chunk:
                        ...
                        text_lower = link['text'].lower()
                        if len(text_lower) < 10 or any(x in text_lower for x in ['login', 'sign up']):
                            continue
                    ...
                    try:
                        ...
                        for link, decision in zip(valid_chunk, batch_results):
                            if decision.get('should_click', False):
                                ...
                                # Log positive finds to UI
                                if self.ui_callback:
                                    self.ui_callback('log', {
                                        'type': 'decision',
                                        'message': f'✓ Found: {link["text"][:40]}...',
                                        'details': f"Confidence: {confidence}"
                                    })
                    except Exception as e:
                        if self.ui_callback:
                            self.ui_callback('log', {'type': 'error', 'message': f'Batch error: {e}'})
