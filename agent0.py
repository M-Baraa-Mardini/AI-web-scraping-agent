        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if response.status_code == 200:
                ...
                hermes_models = [name for name in model_names if any(
                    alias in name.lower() for alias in ["hermes", "hermes3", "hermes-3"]
                )]
...
class LinkEvaluator:
    ...
    async def evaluate_link_from_title(self, topic: str, link_text: str, url: str) -> Dict:
        """Evaluate if a link is worth clicking based on title alone"""
        cache_key = f"{topic}_{link_text}_{url}"
        if cache_key in self.cache:
            return self.cache[cache_key]
        
        logger.log("decision", f"Evaluating link: {link_text[:50]}...")
        
        prompt = f"""Evaluate if this link is relevant to research topic: "{topic}"
        ...
        Instructions:
        1. Analyze if this link is likely to contain information about "{topic}"
        2. Consider the link text and URL patterns
        3. Return a decision and confidence score
