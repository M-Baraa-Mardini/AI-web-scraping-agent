...
class ModernUIAgent(SmartResearchAgent):
    ...
    async def research_ui(self, topic, start_url, countries, strict_countries, max_batches=0):
        """Main Loop with batch limiting and progress updates."""
        if self.ui_callback:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for i in range(batches_to_process):
                    ...
                    # Progress update
                    fraction = 0.1 + 0.4 * (links_evaluated / total_selected)  # 0.1 to 0.5 range
                    self.ui_callback('progress', {
                        'stage': f'Evaluating: {links_evaluated}/{total_selected}',
                        'fraction': fraction
                    })
                ...
                for link_info in promising_links:
                    ...
                    self.ui_callback('progress', {
                        'stage': f'Visiting: {visited}/{len(promising_links)}',
                        'fraction': fraction
                    })
...
class ResultsDialog(tk.Toplevel):
    """Dialog to view, select, and export research results (improved UI)."""
    def __init__(self, parent, results):
        ...
        # Mapping tree item iid -> result index
        self.item_to_idx = {}
    ...
    def on_select(self, event=None):
        ...
        self.counter_var.set(f"{len(selected)} selected / {len(self.results)} total")

        if selected:
            first = selected[0]
            idx = self.item_to_idx.get(first)
            if idx is not None:
    ...
    def open_selected_url(self, event=None):
        selected = self.tree.selection()
        if selected:
            idx = self.item_to_idx.get(selected[0])
            if idx is not None:
    ...
    def export_results(self):
        ...
        selected_results = [self.results[self.item_to_idx[item_id]] for item_id in selected_items]
